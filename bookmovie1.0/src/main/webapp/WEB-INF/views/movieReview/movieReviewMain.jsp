...
<table>
	<tr>
		<td align="right">
			...
			<option value="m.title">영화제목</option></select></td>
...
review.list = function(pageNo) {
	$.getJSON('${root}/m-review/list/' + pageNo, function(data) {
...
review.listForm = function(data) {
	...
	var table = '<table id="list" style="width: 70%;" border="1px solid black">';
		...
		table += '<td border="1px solid black" style="background-color: YELLOW" colspan="2">영화 제목</td>';
	...
	$.each(data.list,function(){
		...
		table += '<td style="width: 104px; height:150px;" border="1px solid black"><img src="${context}/image/movieimage/'+this.poster+'" style="width: 104px; height:150px;" /></td>';
		table += '<td border="1px solid black">'+this.title+'</td>';
	...
	$('#search-sel').on("change",function(data){
		...
		console.log('검색분류 선택'+selectedOption);
		 $('#searching').click(function() {
			 var word = $('#word').val();
			$.getJSON('${root}/m-review/search/' + selectedOption + '/' + word+ '/1',function(data) {
				if (data.size == 0) {
					review.noData();
				} else {
...
review.detail = function(brdNo) {
	$.getJSON('${root}/m-review/detail/'+brdNo,function(data) {
...
review.detailForm = function(data) {
	...
	table += '<tr>';
	table += '<td>영화 제목 : '+data.title+'&nbsp&nbsp&nbsp&nbsp 작성자 : '+data.userId+'</td></tr>';
...
review.btnDel = function(data) {
	...
	$('#btnDel').click(function() {
		...
		if (userId == data.userId) {
			$.getJSON('${root}/m-review/del/'+data.brdNo,function() {
					alert('글이 삭제되었습니다.');
					review.list(1);
				});
		} else {
...
review.updateForm = function(data) {
	$('#box').empty();
	$('#box').append('<form action="${root}/m-review/update/'+data.brdNo+'" id="frmUpdate" name="frmUpdate" method="GET">');
	var table = '<table class="tab" style="width: 700px; margin: 0 auto;">';
	table += '<tr>';
	table += '<td>영화 제목 : '+data.title+'&nbsp&nbsp&nbsp&nbsp 작성자 : '+data.userId+'</td></tr>';
	table += '<tr><td style="width: 100%; height: 50px;">';
