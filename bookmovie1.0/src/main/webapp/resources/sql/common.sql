...
CREATE TABLE BOOK_AVG_GRADE(
    ...
    CONSTRAINT BOOK_GRADE_AVG_FK FOREIGN KEY(STAT_ID) REFERENCES BOOK_STAT_META(STAT_ID),
    CONSTRAINT BOOK_GRADE_AVG_FK_BOOK FOREIGN KEY(BOOK_SERIAL_NO) REFERENCES BOOK(BOOK_SERIAL_NO)
);
...
CREATE TABLE MOVIE_AVG_GRADE(
    ...
    MOVIE_CODE VARCHAR2 (10),
    AVG_POINT NUMBER DEFAULT 0,
    CONSTRAINT MOVIE_GRADE_AVG_FK_STAT_ID FOREIGN KEY(STAT_ID) REFERENCES MOVIE_STAT_META(STAT_ID),
...
INSERT INTO BOARD VALUES(BRD_NO.NEXTVAL,'abc115', 1002 ,'코미디', '미쓰 와이프 너무 재밌어요' ,'너무 재미있어요 ㅎㅎ' , to_date('2015-08-16', 'yyyy-mm-dd'), 152 );INSERT INTO BOARD VALUES(BRD_NO.NEXTVAL,'abc115', 1002 ,'어드벤처,SF', '쥬라기월드 진짜 재미있어요' ,'너무 재미있어요 ㅋ' , to_date('2015-04-10', 'yyyy-mm-dd'), 152 );
...
INSERT INTO BOARD VALUES(BRD_NO.NEXTVAL,'abc113', 1002 ,'코미디', '픽셀 재미있어요 ㅋㅋ' ,'소재가 너무 독특해서 좋았어요.' , to_date('2015-04-23', 'yyyy-mm-dd'), 152 );INSERT INTO BOARD VALUES(BRD_NO.NEXTVAL,'abc113', 1002 ,'호러', '인시디어스3 재미있어요~' ,'재미있었다 ㅎㅎ' , to_date('2015-05-25', 'yyyy-mm-dd'), 152 );
...
INSERT INTO MOVIE(MOVIE_CODE,GENRE,TITLE,VIEW_TIME,POSTER,REG_DATE,BACKGROUND,IFRAME,MOVIE_INFO)
VALUES('M021','액션','베테랑','2015.08.05','veteran.jpg','2015.08.05','veteranBack.jpg','<iframe width="800" height="515" src="https://www.youtube.com/embed/El8gg8XH7X4" frameborder="0" allowfullscreen></iframe>','한 번 꽂힌 것은 무조건 끝을 보는 행동파 ‘서도철’(황정민), 20년 경력의  승부사 ‘오팀장’(오달수), 위장 전문 홍일점 ‘미스봉’(장윤주), 육체파 ‘왕형사’(오대환), 막내 ‘윤형사’(김시후)까지 겁 없고, 못 잡는 것 없고, 봐주는 것 없는 특수 강 력사건 담당 광역수사대. 오랫동안 쫓던 대형 범죄를 해결한 후 숨을 돌리려는 찰나, 서도철은 재벌 3세 ‘조태오’(유아인)를 만나게 된다. 세상 무서울 것 없는 안하무인의 조태 오와 언제나 그의 곁을 지키는 오른팔 ‘최상무’(유해진). 서도철은 의문의 사건을 쫓던 중 그들이 사건의 배후에 있음을 직감한다. 건들면 다친다는 충고에도 불구하고 포기하지  않는 서도철의 집념에 판은 걷잡을 수 없이 커져가고 조태오는 이를 비웃기라도 하듯 유유히 포위망을 빠져 나가는데… 베테랑 광역수사대 VS 유아독존 재벌 3세 2015년 여름, 자존 심을 건 한판 대결이 시작된다!'); 
INSERT INTO MOVIE(MOVIE_CODE,GENRE,TITLE,VIEW_TIME,POSTER,REG_DATE,BACKGROUND,IFRAME,MOVIE_INFO)
VALUES('M022','액션','암살','2015.07.22','assassination.jpg','2015.07.22','assassinationBack.jpg','<iframe width="800" height="515"  src="https://www.youtube.com/embed/LllIFYytkfs" frameborder="0" allowfullscreen></iframe>','1933년 조국이 사라진 시대 대한민국 임시정부는 일본 측에 노출되지 않은  세 명을 암살작전에 지목한다. 한국 독립군 저격수 안옥윤, 신흥무관학교 출신 속사포, 폭탄 전문가 황덕삼! 김구의 두터운 신임을 받는 임시정부 경무국 대장 염석진은 이들을 찾 아 나서기 시작한다. 암살단의 타깃은 조선주둔군 사령관 카와구치 마모루와 친일파 강인국. 한편, 누군가에게 거액의 의뢰를 받은 청부살인업자 하와이 피스톨이 암살단의 뒤를 쫓 는데... 친일파 암살작전을 둘러싼 이들의 예측할 수 없는 운명이 펼쳐진다!'); 
INSERT INTO MOVIE(MOVIE_CODE,GENRE,TITLE,VIEW_TIME,POSTER,REG_DATE,BACKGROUND,IFRAME,MOVIE_INFO)
VALUES('M023','로맨스','뷰티 인사이드','2015.08.20','beautyinside.jpg','2015.08.20','beautyinsideback.jpg','<iframe width="800" height="515"  src="https://www.youtube.com/embed/-dm_XE9qdjU" frameborder="0" allowfullscreen></iframe>','남자, 여자, 아이, 노인.. 심지어 외국인까지! 자고 일어나면 매일 다른 모 습으로 변하는 남자, ‘우진’. 그에게 처음으로 비밀을 말하고 싶은 단 한 사람이 생겼다. 드디어 D-DAY! ‘우진’은 그녀에게 자신의 마음을 고백하기로 하는데… “초밥이 좋아요? 스테이크가 좋아요? 사실.. 연습 엄청 많이 했어요. 오늘 꼭 그쪽이랑 밥 먹고 싶어서…”'); 
INSERT INTO MOVIE(MOVIE_CODE,GENRE,TITLE,VIEW_TIME,POSTER,REG_DATE,BACKGROUND,IFRAME,MOVIE_INFO)
...
VALUES (1, '사용자의 월별 독서량');
INSERT INTO BOOK_STAT_TYPE(STAT_TYPE_CODE, STAT_TYPE_NAME)
VALUES (2, '각 도서의 평균 평점');
